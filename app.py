...
# Sabit Google Sheets Bilgileri (Herkese Açık Yeni Tablonuz)
...
# CSS: Sitenin arka planını sarı yapan, yazıları siyaha sabitleyen ve mobil uyumluluk sağlayan tasarım
st.markdown(
    """
...
# --- DIRECT GOOGLE API (GSPREAD) BAĞLANTISI (KOD İÇİNE GÖMÜLÜ ŞİFRE) ---
ROBOT_KIMLIGI = {
...
if st.button(buton_metni, type="primary"):
    with st.spinner(yukleniyor_metni):
        ...
        # Orijinal Zengin Anahtar Kelime Listesi
        terimler = [
...
if 'son_aramalar' in st.session_state:
    ...
    for i, makale in enumerate(st.session_state.son_aramalar):
        try:
            ...
            # Gelişmiş Merkez Çekme Algoritması
