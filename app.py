...
# Sitenin arka planını sarı yapan, yazıları siyaha sabitleyen ve hafif kemik silüeti ekleyen CSS
st.markdown(
    """
    <style>
    ...
    .stApp {
        ...
        /* Kemik silüeti desenini SVG olarak arka plana gömüyoruz, gözü yormasın diye opaklığı %5'e çektik */
    ...
    /* Üst taraftaki başlık ve açıklamaların her koşulda koyu renk/siyah kalmasını sağlar */
    ...
    /* Kartın içindeki başlık ve yazıların renkleri */
    .makale-kart h3 {
        color: #2C3E50 !important;
        margin-top: 0;
    }
...
# Başlıklar artık yukarıdaki CSS sayesinde her zaman koyu renk okunacak
            ...
            for i, makale in enumerate(makale_detaylari['PubmedArticle']):
                try:
                    ...
                    <div class="makale-kart">
                        <h3 style="margin-top: 0; font-size: 1.25em;">{i+1}. {baslik}</h3>
                        <p style="margin: 5px 0; font-size: 0.9em;"><b>Dergi:</b> {dergi}</p>
                        <p style="margin: 5px 0; font-size: 0.9em;"><b>Merkez:</b> {merkez}</p>
                        <hr style="border: 0; border-top: 1px solid #ECF0F1; margin: 12px 0;">
                    """
                    
                    if yontem:
                        kart_icerigi += f"<p style='margin: 10px 0;'><b>[Yöntem]:</b> {' '.join(yontem)}</p>"
                    if sonuc:
                        kart_icerigi += f"<p style='margin: 10px 0; color: #27AE60;'><b>[Sonuç]:</b> {' '.join(sonuc)}</p>"
                    if not yontem and not sonuc and duz_abstract:
                        kart_icerigi += f"<p style='margin: 10px 0;'><b>[Özet]:</b> {' '.join(duz_abstract)}</p>"
